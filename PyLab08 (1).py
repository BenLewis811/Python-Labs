...
# in miles
def plot_finalmap(loc, x, y, mapTitle, mapColor):
    """
    This function plots the map desired when called.
    Parameters:
    loc: numpy array of the names of the locations
    x: numpy array of the x values of the locations (days journey)
    y: numpy array of the y values of the locations (days journey)
    mapTitle: String containing the title of the map
    mapColor: String containing the color value for the path and markers

    The function is a void function and has no return value. After the function is called,
    it will display the graph with these elements:
    Background map image from lab08-background.png, Connected path with star markers, Location 
    annotations with white text, Axis labels and title, 8x8 day journey coordinate system.
    """

    plt.figure()
    
    # Load and display background first
    background_image = plt.imread("lab08-background.png")
    plt.imshow(background_image, aspect='auto', extent=[0,8,0,8], zorder=0)
    
    # Plot path with different color
    plt.plot(x, y, color=mapColor, marker='*', linestyle='-', 
            markerfacecolor='yellow', zorder=3)
    
    # Add annotations with contrasting text
    for i in range(len(loc)):
        plt.annotate(
            loc[i], 
            (x[i], y[i]),
